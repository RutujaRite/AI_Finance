...
async function main() {
  const bankCode = 'FIBE';
  const bankName = 'Fibe (EarlySalary)';
  const masterFileName = 'Fibe_Master_Policy.txt';
  ...
  // Get all policy rules for FIBE (all loan types)
  ...
  console.log('FIBE policy rules (all loan types):', rulesRes.rows.length);
  ...
  if (existingBpf.rowCount > 0) {
    ...
    `, [
      ...
      fileStats.size,
      1,
      'Master policy rulebook for Fibe (EarlySalary) Personal Loan',
      masterText,
      JSON.stringify({ is_master_policy: true, is_unified_text: true, document_type: 'Main Eligibility Policy', is_active: true })
    ]);
  ...
  // Deactivate all old FIBE source documents in bank_policy_files (do NOT delete physical files)
  ...
  console.log('\n=== Verification: FIBE Policy Rules (all loan types) ===');
  ...
  if (attachments.length === 1 && attachments[0].file_name === masterFileName) {
    console.log('\n✅ SUCCESS: FIBE Bank has exactly one policy document:', masterFileName);
  } else {
  ...
  // Verify no other active documents remain for FIBE
  ...
  if (otherActive.rowCount === 0) {
    console.log('\n✅ No other active documents remain for FIBE.');
  } else {
    console.log('\n⚠️ WARNING: Other active documents found for FIBE:', otherActive.rows);
  }
}
