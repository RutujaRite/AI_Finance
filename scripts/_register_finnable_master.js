...
async function main() {
  const bankCode = 'FINNABLE';
  const bankName = 'Finnable Credit';
  const masterFileName = 'Finnable_Credit_Master_Policy.txt';
  ...
  // Get all policy rules for FINNABLE (all loan types)
  ...
  console.log('FINNABLE policy rules (all loan types):', rulesRes.rows.length);
  ...
  if (existingBpf.rowCount > 0) {
    ...
    `, [
      ...
      fileStats.size,
      1,
      'Master policy rulebook for Finnable Credit Personal Loan',
      masterText,
      JSON.stringify({ is_master_policy: true, is_unified_text: true, document_type: 'Main Eligibility Policy', is_active: true })
    ]);
  ...
  // Deactivate all old FINNABLE source documents in bank_policy_files (do NOT delete physical files)
  ...
  console.log('\n=== Verification: FINNABLE Policy Rules (all loan types) ===');
  ...
  if (attachments.length === 1 && attachments[0].file_name === masterFileName) {
    console.log('\n✅ SUCCESS: FINNABLE Bank has exactly one policy document:', masterFileName);
  } else {
  ...
  // Verify no other active documents remain for FINNABLE
  ...
  if (otherActive.rowCount === 0) {
    console.log('\n✅ No other active documents remain for FINNABLE.');
  } else {
    console.log('\n⚠️ WARNING: Other active documents found for FINNABLE:', otherActive.rows);
  }
}
