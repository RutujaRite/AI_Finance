...
async function main() {
  ...
  const masterFileName = 'HDFC_Bank_Master_Policy.txt';
  ...
  // Get all policy rules for HDFC (all loan types)
  ...
  console.log('HDFC policy rules (all loan types):', rulesRes.rows.length);
  ...
  if (existingBpf.rowCount > 0) {
    ...
    `, [
      ...
      fileStats.size,
      1,
      'Master policy rulebook for HDFC Bank Personal Loan',
      masterText,
      JSON.stringify({ is_master_policy: true, is_unified_text: true, document_type: 'Main Eligibility Policy', is_active: true })
    ]);
  ...
  // Deactivate all old HDFC source documents in bank_policy_files (do NOT delete physical files)
  ...
  console.log('\n=== Verification: HDFC Policy Rules (all loan types) ===');
  ...
  if (attachments.length === 1 && attachments[0].file_name === masterFileName) {
    console.log('\n✅ SUCCESS: HDFC Bank has exactly one policy document:', masterFileName);
  } else {
  ...
  // Verify no other active documents remain for HDFC
  ...
  if (otherActive.rowCount === 0) {
    console.log('\n✅ No other active documents remain for HDFC.');
  } else {
    console.log('\n⚠️ WARNING: Other active documents found for HDFC:', otherActive.rows);
  }
}
