...
export async function processEligibilityFlow(
  ...
  if (activeState && activeState.in_eligibility_flow) {
    ...
    if (currentExpectedField !== "companyName" && currentExpectedField !== "companySelection" && currentExpectedField !== "loanType") {
      await clearConversationState(pool, conversationId);
    ...
    // Handle loan type selection step
    if (currentExpectedField === "loanType") {
      const lower = userMessage.trim().toLowerCase();
      let loanType = "Personal";
      if (/home\s*loan/i.test(lower)) loanType = "Home";
      else if (/business\s*loan/i.test(lower)) loanType = "Business";
      else if (/auto\s*loan|car\s*loan/i.test(lower)) loanType = "Car";

      await setConversationState(pool, conversationId, {
        in_eligibility_flow: true,
        applicant: { loanType },
        expected_field: "monthlyIncome",
      });

      return {
        reply: `Got it! You need a **${loanType}** loan.\n\nTo check your eligibility, I'll need a few details:\n\n1. What is your monthly take-home salary (in ₹)?\n2. What is your CIBIL credit score?\n3. What loan amount do you need (in ₹)?\n4. What is your preferred tenure (in months)?\n\nPlease reply with these details to get started.`,
        isFinished: false,
        applicant: { loanType },
      };
    }
  ...
  // The LLM already decided this is an eligibility request. Try to resolve
  // a valid company name from the message so the wizard can skip the
  // company step when the user provided it inline.
  try {
    const compRes = await searchCompany(userMessage);
...
/**
 * Creates a new loan eligibility intent flow that asks the user for required fields
 * without searching company data. Starts with loan type selection, then proceeds
 * through the standard eligibility wizard steps.
 */
export async function createLoanIntentFlow(
  ...
  let loanType = "Personal";
  if (/home\s*loan/i.test(normalized)) loanType = "Home";
  else if (/business\s*loan/i.test(normalized)) loanType = "Business";
  else if (/auto\s*loan|car\s*loan/i.test(normalized)) loanType = "Car";

  await setConversationState(pool, conversationId, {
    in_eligibility_flow: true,
    applicant: {},
    expected_field: "loanType",
    loanType,
  });
...
/**
 * Format a deterministic eligibility result as markdown.
 * The LLM owns the final user-facing response; this function only
 * converts the deterministic engine's output into plain text.
 */
...
/**
 * Structured eligibility entry point for the LLM tool-calling loop.
 *
 * The LLM decides a user message is an eligibility request and calls this
 * function with the structured parameters it extracted. The deterministic
 * engine remains the sole authority for the financial decision; the LLM
 * only explains the returned result.
 *
 * Returns a plain-text report ready to hand back to the LLM for summarization.
 */
export async function evaluateEligibilityFromTool(
  input: DeterministicApplicantInput,
  poolOverride?: any
): Promise<string> {
  const result = await calculateDeterministicEligibility(input, poolOverride);
  const bankName = result.bankName || input.bankName || "the requested bank";
  return formatCalculatorResult(result, bankName);
}
...
/**
 * Calculate EMI using the standard annuity formula:
 *   EMI = P × r × (1+r)^n / ((1+r)^n - 1)
 *
 * @param principal  Loan amount (₹)
 * @param rate       Annual interest rate (e.g. 11.5 for 11.5%)
...
/**
 * Format an EMI result as a markdown table for the LLM to present.
 */
export function formatEmiResult(input: EmiInput, result: EmiResult): string {
  return (
