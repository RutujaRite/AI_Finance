...
export interface BankManagerSearchParams {
  ...
  /**
   * Pincode supplied by the LLM.
   *
   * Example:
   * "411001"
   *
   * The search will look for this value in the location field
   * since the bank_managers table stores pincodes in the
...
/* -------------------------------------------------
      ...
      *
      * Exact match on the city column only.
      * The LLM supplies the city; we match it precisely.
      * We do NOT search district/state/location with
      * LIKE — that would return unrelated locations.
      * ------------------------------------------------- */

    if (city) {
      queryParams.push(normalizeValue(city));

      query += `
        AND LOWER(TRIM(COALESCE(city, ''))) = LOWER($${queryParams.length})
      `;
    }
    ...
    /* -------------------------------------------------
      * PINCODE FILTER
      *
      * Pincode is stored in the location field (since
      * bank_managers has no dedicated pincode column).
      * We match the pincode as an exact word within the
      * location string.
      * ------------------------------------------------- */

    if (params.pincode) {
      const pincodeValue = normalizeValue(params.pincode);
      queryParams.push(pincodeValue);
      const p = `$${queryParams.length}`;

      query += `
        AND (
          LOWER(COALESCE(location, '')) LIKE LOWER(${p} || '%') ESCAPE '\\'
          OR LOWER(COALESCE(extra_info->>'pincode', '')) = LOWER(${p})
        )
      `;
    }
