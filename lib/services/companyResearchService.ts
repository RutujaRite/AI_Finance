// lib/services/companyResearchService.ts
...
function truncateBullet(text: string, maxLen = 100): string {
  let cleaned = cleanTavilyText(text)
    .replace(/^[-*•\d.)\s]+/, '')
    .replace(/^(and|with|it|which|who|also|that|as)\s+/gi, '')
    .trim();
  if (!cleaned) return '';
...
function filterRelevantPassages(companyName: string, tavilyData: any) {
  const companyTokens = companyName.toLowerCase().split(/\s+/).filter(t => t.length > 2 && !/^(company|solutions|technologies|services|pvt|ltd|limited|inc|corp|corporation|group|india|international)$/gi.test(t));
...
function detectIndustry(fullText: string, companyName: string): string {
  ...
  if (/\b(incredit|bajaj finance|muthoot|cred|paytm|lendingkart|moneytap|tata capital)\b/gi.test(name) || (finScore > itScore && finScore >= 2)) {
    return 'Financial Services & Fintech / Lending';
  }

  if (/\b(tcs|tata consultancy|ibm|infosys|wipro|cognizant|accenture|hcl|tech mahindra|capgemini|oracle|microsoft|google|amazon|salesforce)\b/gi.test(name) || (itScore >= finScore && itScore > 0)) {
    return 'Information Technology & Consulting Services';
  }

  if (/\b(fintech|lending|loan|loans|mortgage|credit|nbfc|banking|wealth management|financial services)\b/gi.test(text)) {
    return 'Financial Services & Banking / Lending';
  }
  if (/\b(it services|technology|software|cloud|digital transformation|cybersecurity|ai)\b/gi.test(text)) {
    return 'Information Technology & Digital Services';
  }
  return 'Enterprise & Business Services';
}
function extractWebsite(companyName: string, results: any[], answer: string, passages: string[]): string | null {
  const nameTokens = companyName.toLowerCase().replace(/[^a-z0-9\s]/g, '').split(/\s+/).filter(t => t.length >= 2 && !/^(company|solutions|technologies|services|pvt|ltd|limited|inc|corp|corporation|group|india|international)$/gi.test(t));
  const coreToken = nameTokens[0] || companyName.toLowerCase().slice(0, 3);
  const excludedDomains = /google|bing|yahoo|wikipedia|scribd|zoominfo|linkedin|facebook|twitter|instagram|youtube|github|tracxn|crunchbase|glassdoor|indeed|cbinsights|globaldata|reuters|bloomberg|forbes|highperformr|tradewindsdr|clay\.com|bccresearch/gi;

  if (answer) {
    const directUrlMatch = answer.match(/\b(?:https?:\/\/)?(?:www\.)?([a-zA-Z0-9-]+\.(?:com|in|org|net|io|co|ai|tech))\b/gi);
    if (directUrlMatch) {
...
function extractHQ(text: string, passages: string[]): string {
  const combined = (text + ' ' + passages.join(' '));
  const m1 = combined.match(/(?:headquartered|based|hq|registered office|corporate office)\s+(?:in|at)\s+([A-Za-z0-9\s,.-]+?(?:Mumbai|Bengaluru|Bangalore|Armonk|New York|Delhi|Pune|Hyderabad|Chennai|California|India|USA|United States))/gi);
  if (m1 && m1[1]) {
    const loc = cleanTavilyText(m1[1]).replace(/^(in|at)\s+/gi, '').replace(/\s*(and|operating|with|is|reachable|\().*$/gi, '').trim();
    if (loc.length > 2 && loc.length < 80) return loc;
  }
  const m2 = combined.match(/headquartered\s+(?:at|in)\s+([^;.,\n]+(?:,\s*[^;.,\n]+){0,2})/gi);
  if (m2 && m2[1]) {
    const loc = cleanTavilyText(m2[1]).replace(/\s*(and|operating|with|is|reachable|\().*$/gi, '').trim();
    if (loc.length > 2 && loc.length < 80) return loc;
  }
  return 'Not specified in live search';
}
function extractOverview(name: string, answer: string, passages: string[]): string {
  if (answer) {
    const firstSent = answer.split(/[;.]/)[0].replace(/\s*\([^)]*\)/g, ' ');
    return truncateBullet(firstSent, 160);
  }
  for (const p of passages) {
    if (new RegExp(`${name}.{0,30}(is|provides|offers)`, 'gi').test(p)) {
      return truncateBullet(p, 160);
    }
  }
  return `${name} is an active enterprise organization.`;
}
function extractLeadership(text: string, passages: string[]): string[] {
  const items: string[] = [];
  const seen = new Set();
  const combined = (text + ' ' + passages.join(' '));
  ...
  if (text) {
    ...
    if (m && m[1]) {
      const clean = truncateBullet(m[1].replace(/\s*(who|which|and)\s*$/gi, ''), 80);
      if (clean.length > 4) {
        items.push(clean);
  ...
  for (const match of leadMatches) {
    const full = truncateBullet(match[0].replace(/\s+(and|who|with|is)$/gi, ''), 50);
    const key = full.toLowerCase().slice(0, 15);
    if (full.length > 6 && !seen.has(key) && !/company|details|overview|snapshot|report|history|public|management|services|products/gi.test(full)) {
      seen.add(key);
      items.push(full);
...
function extractServicesProducts(name: string, text: string, passages: string[]): string[] {
  const items: string[] = [];
  const seen = new Set();
  ...
  if (text) {
    ...
    if (servMatch && servMatch[1]) {
      const parts = servMatch[1].split(/,\s*(?:with proprietary|including|such as|and)\s*/gi);
      for (const pt of parts) {
        const item = truncateBullet(pt, 90);
        const key = item.toLowerCase().slice(0, 20);
        if (item.length > 8 && !seen.has(key) && !item.toLowerCase().includes(name.toLowerCase() + ' is') && !/view|overview|snapshot/gi.test(item)) {
          seen.add(key);
          items.push(item);
  ...
  if (items.length < 2) {
    for (const p of passages) {
      if (/\b(services|products|solutions|offerings|consulting|platforms|cloud|loans|lending|software)\b/gi.test(p)) {
        const item = truncateBullet(p, 90);
        const key = item.toLowerCase().slice(0, 20);
        if (item.length > 15 && !seen.has(key) && !item.toLowerCase().includes(name.toLowerCase() + ' is') && !/view|snapshot|overview|report/gi.test(item)) {
          seen.add(key);
          items.push(item);
          if (items.length >= 3) break;
        }
...
function extractLocationsScale(text: string, passages: string[]): string[] {
  const items: string[] = [];

  const m1 = text.match(/(?:operates in|presence in|locations across|footprint of)\s+([0-9]+\s+(?:locations|countries|cities)[^;.,\n]*)/gi);
  if (m1 && m1[0]) {
    items.push(truncateBullet(m1[0], 90));
  }

  const m2 = text.match(/(?:workforce of|employs (?:roughly|over)?)\s+([0-9,]+\s+(?:employees|people|professionals)[^;.,\n]*)/gi);
  if (m2 && m2[0]) {
...
function extractBusinessInfo(text: string, passages: string[]): string[] {
  const items: string[] = [];

  const revMatch = text.match(/(?:reported revenue of|revenues of|market capitali[sz]ation (?:of|near|around))\s+([^;.,\n]+)/gi);
  if (revMatch && revMatch[0]) {
    items.push(truncateBullet(revMatch[0], 90));
  }

  const listMatch = text.match(/(?:traded as|publicly traded on|listed on)\s+([^;.,\n]+)/gi);
  if (listMatch && listMatch[0]) {
    items.push(truncateBullet(listMatch[0], 80));
  }

  if (items.length === 0) {
    const fMatch = text.match(/(?:founded in [0-9]{4}|trusted .* since [0-9]{4})/gi);
    if (fMatch) items.push(truncateBullet(fMatch[0], 60));
  }

  return items.slice(0, 2);
}
function extractRecentDevelopments(text: string, passages: string[]): string[] {
  const items: string[] = [];
  if (text) {
  ...
  for (const p of passages) {
    if (items.length >= 2) break;
    if (/\b(recently launched|announced|partnered with|acquired|expanded|new business units|new platform)\b/gi.test(p)) {
      const item = truncateBullet(p, 90);
      if (item.length > 20 && !items.includes(item) && !/overview|snapshot|view/gi.test(item)) {
        items.push(item);
      }
    }
  }
...
async function fetchLiveCompanySummary(companyName: string): Promise<string> {
  ...
  try {
    ...
    const summary = buildCompanyProfileFromTavilyData(companyName, data);
...
function buildCompanyProfileFromTavilyData(companyName: string, tavilyData: any): string {
  ...
  results.forEach(r => {
    if (r.url && /^https?:\/\//gi.test(r.url) && !seenUrls.has(r.url)) {
      seenUrls.add(r.url);
      sources.push(r.url);
    }
  });
...
function formatCompanyProfile(profile: any): string {
  if (!profile) return '';
  ...
  if (profile.services && profile.services.length > 0) {
    profile.services.forEach((s: string) => lines.push(`- ${s}`));
  } else {
  ...
  if (profile.leadership && profile.leadership.length > 0) {
    profile.leadership.forEach((l: string) => lines.push(`- ${l}`));
  } else {
    lines.push(`- Not specifically detailed in live search.`);
  }
  lines.push('');
  ...
  if (profile.locations && profile.locations.length > 0) {
    profile.locations.forEach((loc: string) => lines.push(`- ${loc}`));
  } else {
    lines.push(`- Not specifically detailed in live search.`);
  }
  lines.push('');
  ...
  if (profile.businessInfo && profile.businessInfo.length > 0) {
    profile.businessInfo.forEach((b: string) => lines.push(`- ${b}`));
  } else {
    lines.push(`- Not specifically detailed in live search.`);
  }
  lines.push('');
  ...
  if (profile.recentDevelopments && profile.recentDevelopments.length > 0) {
    profile.recentDevelopments.forEach((r: string) => lines.push(`- ${r}`));
  } else {
    lines.push(`- Not specifically detailed in live search.`);
  }
  lines.push('');
  ...
  if (profile.sources && profile.sources.length > 0) {
    lines.push(`🔗 Sources:`);
    profile.sources.forEach((src: string) => lines.push(`- ${src}`));
  }
