// services/companyResearchService.js
...
function truncateBullet(text, maxLen) {
...
function filterRelevantPassages(companyName, tavilyData) {
  const companyTokens = companyName.toLowerCase().split(/\s+/).filter(function(t) {
    return t.length > 2 && !/^(company|solutions|technologies|services|pvt|ltd|limited|inc|corp|corporation|group|india|international)$/gi.test(t);
  });
...
function extractWebsite(companyName, results, answer, passages) {
  const nameTokens = companyName.toLowerCase().replace(/[^a-z0-9\s]/g, '').split(/\s+/).filter(function(t) {
    return t.length >= 2 && !/^(company|solutions|technologies|services|pvt|ltd|limited|inc|corp|corporation|group|india|international)$/gi.test(t);
  });
...
function formatCompanyProfile(profile) {
  ...
  if (profile.services && profile.services.length > 0) {
    profile.services.forEach(function(s) {
      lines.push(`- ${s}`);
    });
  } else {
  ...
  if (profile.leadership && profile.leadership.length > 0) {
    profile.leadership.forEach(function(l) {
      lines.push(`- ${l}`);
    });
  } else {
    lines.push(`- Not specifically detailed in live search.`);
  ...
  if (profile.locations && profile.locations.length > 0) {
    profile.locations.forEach(function(loc) {
      lines.push(`- ${loc}`);
    });
  } else {
    lines.push(`- Not specifically detailed in live search.`);
  ...
  if (profile.businessInfo && profile.businessInfo.length > 0) {
    profile.businessInfo.forEach(function(b) {
      lines.push(`- ${b}`);
    });
  } else {
    lines.push(`- Not specifically detailed in live search.`);
  ...
  if (profile.recentDevelopments && profile.recentDevelopments.length > 0) {
    profile.recentDevelopments.forEach(function(r) {
      lines.push(`- ${r}`);
    });
  } else {
    lines.push(`- Not specifically detailed in live search.`);
  ...
  if (profile.sources && profile.sources.length > 0) {
    lines.push(`🔗 Sources:`);
    profile.sources.forEach(function(src) {
      lines.push(`- ${src}`);
    });
  }
